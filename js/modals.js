...
function handleLogin(event) {
    ...
    setTimeout(() => {
        ...
        if (demoAccounts[email] && demoAccounts[email].password === password) {
            ...
            if (foundUser) {
                // For demo purposes, we'll check against a simple stored password
    ...
    setTimeout(() => {
        try {
            ...
            const newUser = {
                ...
                password: formData.get('password'), // In production, this would be hashed
                role: 'user',
            ...
            // Show profile completion prompt after delay
            setTimeout(() => {
                if (confirm('Would you like to complete your profile now? This helps other researchers connect with you.')) {
                    showProfileEditor();
                }
            }, 5000);
...
function updatePasswordStrength(password, targetElement) {
    ...
    const strengthColor = ['#ff4757', '#ff6b7a', '#ffa726', '#66bb6a', '#4caf50'][strength];
...
// Enhanced UGC Editor with rich text support
function createUGCEditorForm(section = '', contentType = '', item = null) {
    const editorContent = document.querySelector('.ugc-editor-content');
    if
